...
## SCH-011. GET /bookings/{id}: negative_data_rejection

**Вердикт:** ложное срабатывание

Операция не принимает тело запроса. Чек мутирует схему, для которой
нет входных данных кроме валидного path-параметра. Отключён точечно
для этой операции.

**Проверено:** запрос с несуществующим id возвращает 404
(покрыто тестом test_nonexist_booking_returns_404).